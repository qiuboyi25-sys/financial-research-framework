...
它不是一份只保留最终收益曲线的策略代码，而是把研究拆成数据契约、时点控制、因子构造、横截面预处理、统计评价、组合权重、成交延迟和报告输出等独立模块。仓库提供一个完全离线的换手率因子示例，可以从合成日行情开始，一条命令生成因子缓存、字段说明、统计结果和自包含 HTML 报告。
...
这些是可以复用和单独测试的“策略零件”。公开版没有保留依赖受限数据和特定证券清单的完整多资产策略配置，因此不能把本仓库描述为已经提供真实多资产策略回测结果。
...
| 真实回测HTML、CSV和中间缓存 | 可能间接泄露样本、标的、路径和数据范围 | 输出由使用者本地重新生成，不进入Git |
...
- 多资产择时组件已经单元测试，但公开版没有完整真实策略配置和实证绩效。

因此，当前版本更准确的定位是：

> 一个已经跑通、能够发现基础错误的公开研究内核，加上一个可复现的单因子最小示例；不是完整的量化产品或经过验证的投资策略。
